...
/// <summary>A request that has passed validation and been decoded into bytes.</summary>
...
public static class RenderLabelRequestDecoder
{
        ...
        out List<string> errors)
    {
        ...
        if (text.Count + images.Count > options.MaxFieldCount)
            errors.Add($"A request may populate at most {options.MaxFieldCount} fields.");

        foreach (var name in text.Keys.Intersect(images.Keys, StringComparer.Ordinal).Order(StringComparer.Ordinal))
            errors.Add($"Field '{name}' is given both a text and an image value.");
        ...
        decoded = new DecodedLabelRequest(templateBytes, text, decodedImages);
        return true;
    }
